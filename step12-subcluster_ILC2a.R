...
rm(list=ls())
setwd("~/CopyOfjiangshijiu")
dir.create("subcluster_ILC2a")
setwd("subcluster_ILC2a")
load("~/CopyOfjiangshijiu/Doublet/after_doublet_obj.combined.RData")
table(obj.combined$celltype)
sce=obj.combined


sce=sce[,sce@meta.data$celltype %in% c( "ILC2a" )]
...
sce <- FindClusters(sce, resolution = c(seq(.1,1.6,.2)))
...
ggsave("tSNE_ILC2a.pdf",width = 8, height = 6)

sce <- RunUMAP(object = sce, dims = 1:10, do.fast = TRUE)
DimPlot(sce,reduction = "umap",label=T) 
ggsave("umap_ILC2a.pdf", width = 8, height = 6)

head(sce@meta.data)
DimPlot(sce,reduction = "umap",label=T,split.by = 'orig.ident')
ggsave("umap_ILC2a_split.pdf", width = 8, height = 6)

####值得注意的是我们的单细胞亚群取子集，这个seurat对象仍然是来源于多个单细胞样品，
##所以仍然是需要进行某种程度合理的整合哦，我们这里给大家的示范代码是
library(harmony)
...
DimPlot(sce,reduction = "umap",label=T,split.by = "orig.ident") 
ggsave(filename = 'harmony_umap_recluster_by_0.3.pdf',width = 8,height = 6) 
DimPlot(sce,reduction = "umap",label=T,
        group.by = 'orig.ident') 
ggsave(filename = 'harmony_umap_sce_recluster_by_orig.ident.pdf',width = 8,height = 6) 
...
if(T){  #########去除双细胞后重新经过scTYPE命名 
  new.cluster.ids <- c("ILC2a_cluster0","ILC2a_cluster1","ILC2a_cluster2","ILC2a_cluster3")
...
sce@meta.data[["group"]] <- factor(sce@meta.data[["group"]],levels = c("Sham","MIRI","MI"))
sce@meta.data[["celltype"]] <- sce@active.ident
sce$celltype.group <- paste(Idents(sce), sce$group, sep = "_")   ##细胞类型前面加上分组信息

#细胞亚群注释后的UMAP
library(RColorBrewer)   #配色
cols <- c(brewer.pal(8, "Dark2")) #配色
pdf('ILC2a_annotations.pdf',width = 12,height =8)                    
DimPlot(sce, reduction = "umap", label = TRUE,
        label.size = 10,label.box = F,cols = cols,#shape.by = "group",
...
pdf('ILC2a_annotations_group.pdf',width = 36,height = 12)
DimPlot(sce, reduction = "umap", label = TRUE,
...
pdf("stack_bar_ILC2a_3group.pdf",width = 12,height = 8)
p.bar
dev.off()
####################################################################################
##################################单细胞数目条形图
...
pdf("barplot_ILC2a_3group.pdf",width = 16,height = 8)
p1
dev.off()
########################################################################
########################################################
diff.wilcox = FindAllMarkers(sce,only.pos = TRUE, MIn.pct = 0.25,thresh.use = 0.25)
all.markers = diff.wilcox %>% dplyr::select(gene, everything()) %>% subset(p_val<0.05)
write.csv(all.markers,file = "ILC2a_marker.csv")
top5 = all.markers %>% group_by(cluster) %>% top_n(n = 5, wt = avg_log2FC)
DoHeatmap(sce,top5$gene,size=3)
ggsave(filename='ILC2a_top5_heatmap.pdf',height = 8,width = 12)

pdf('dotplot_markers_top5.pdf',width = 12,height = 8)
DotPlot(sce, assay = "RNA", 
        features = top5$gene, 
        cols = c("blue", "red","green"), group.by = "celltype") + 
...
#####拟时序
scRNA<-sce
scRNAsub = sce[,sce@meta.data$orig.ident %in% c("MI")]

dir.create("/home/data/t040243/CopyOfjiangshijiu/subcluster_ILC2a//MI")
setwd("/home/data/t040243/CopyOfjiangshijiu/subcluster_ILC2a//MI")
