...
Idents(obj.combined) <- obj.combined@meta.data[["celltype.group"]]
scRNA<-obj.combined
scRNAsub = obj.combined[,obj.combined@meta.data$celltype.group %in% c("ILCdc_MI","ILCdc_Sham")]

dir.create("ILCdc_MIvsSham")
setwd("ILCdc_MIvsSham")
...
mycds <- orderCells(mycds,root_state =2)    #####注意这里顺序变了
...
##Cluster轨迹分布图
plot2 <- plot_cell_trajectory(mycds, color_by = "celltype.group")+ scale_color_manual(values = colour)+theme_bw()+
  theme(axis.text = element_text(size = 30),
        axis.title = element_text(size =30),
        legend.text = element_text(size = 30),
        legend.title = element_text(size = 30),
...
p2 <- plot_cell_trajectory(mycds, color_by = "celltype.group") + facet_wrap(~celltype.group, nrow = 1)+ scale_color_manual(values = colour)+theme_bw()+
  theme(axis.text = element_text(size = 30),
        axis.title = element_text(size =30),
        legend.text = element_text(size = 30),
        legend.title = element_text(size = 30),
...
View(df)
p4 <- ggplot(df, aes(Pseudotime, colour = celltype.group, fill=celltype.group)) +
...
##########树形图
p2 <- plot_complex_cell_trajectory(mycds, color_by = "celltype.group") + facet_wrap(~celltype.group, nrow = 1)+ scale_color_manual(values = colour)+theme_bw()+
  theme(axis.text = element_text(size = 30),
        axis.title = element_text(size =30),
        legend.text = element_text(size = 30),
        legend.title = element_text(size = 30),
...
#p2 <- plot_genes_violin(mycds[s.genes,], grouping = "celltype.group", color_by = "celltype.group");p2
...
#diff_test <- differentialGeneTest(mycds[disp.genes,], cores = 4,fullModelFormulaStr = "~celltype.group")
...
##可视化：以state/celltype.group/pseudotime进行
#p1 <- plot_genes_in_pseudotime(mycds_subset, color_by = "State")
p2 <- plot_genes_in_pseudotime(mycds_subset, color_by = "celltype.group")
...
genes <- rownames(head(beam_res,4))   ##排名前4的基因
