...
sce=sce[,sce@meta.data$celltype %in% c( "ILCdc" )]
...
ggsave("tSNE_ILCdc.pdf",width = 8, height = 6)

sce <- RunUMAP(object = sce, dims = 1:10, do.fast = TRUE)
DimPlot(sce,reduction = "umap",label=T) 
ggsave("umap_ILCdc.pdf", width = 8, height = 6)

head(sce@meta.data)
DimPlot(sce,reduction = "umap",label=T,split.by = 'orig.ident')
ggsave("umap_ILCdc_split.pdf", width = 8, height = 6)
...
DimPlot(sce,reduction = "umap",label=T,split.by = "orig.ident") 
ggsave(filename = 'harmony_umap_recluster_by_0.1.pdf',width = 8,height = 6) 
DimPlot(sce,reduction = "umap",label=T,
        group.by = 'orig.ident') 
...
if(T){  #########去除双细胞后重新经过scTYPE命名 
  new.cluster.ids <- c("ILCdc_cluster0","ILCdc_cluster1","ILCdc_cluster2","ILCdc_cluster3",
                       "ILCdc_cluster4")
...
pdf('ILCdc_annotations.pdf',width = 12,height =8)                    
...
pdf('ILCdc_annotations_group.pdf',width = 36,height = 12)
DimPlot(sce, reduction = "umap", label = TRUE,
        label.size = 12,label.box = F,cols = cols,shape.by = "group",
...
pdf("stack_bar_ILCdc_3group.pdf",width = 12,height = 8)
p.bar
dev.off()
####################################################################################
##################################单细胞数目条形图
...
pdf("barplot_ILCdc_3group.pdf",width = 16,height = 8)
p1
dev.off()
########################################################################
########################################################
...
write.csv(all.markers,file = "ILCdc_marker.csv")
top5 = all.markers %>% group_by(cluster) %>% top_n(n = 5, wt = avg_log2FC)
DoHeatmap(sce,top5$gene,size=3)
ggsave(filename='ILCdc_top5_heatmap.pdf',height = 8,width = 12)
...
#####拟时序
scRNA<-sce
scRNAsub = sce[,sce@meta.data$orig.ident %in% c("Sham")]

dir.create("/home/data/t040243/CopyOfjiangshijiu/subcluster/Sham")
setwd("/home/data/t040243/CopyOfjiangshijiu/subcluster/Sham")
